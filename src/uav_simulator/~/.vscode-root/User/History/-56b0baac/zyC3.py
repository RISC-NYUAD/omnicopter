...
def generate_launch_description():
    ...
    world_file = os.path.join(uav_simulator_path, 'worlds', f'{world_name.perform({})}.world')
    params_file = os.path.join(uav_simulator_path, 'config', 'omnicopter_params.yaml')
    models_path = os.path.join(uav_simulator_path, 'models')
    ...
    gazebo = ExecuteProcess(
        cmd=[
            ...
            '--gui', gui.perform({}),
            '--pause' if paused.perform({}) == 'true' else ''
        ],
        output='screen'
    )
