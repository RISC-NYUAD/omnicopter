...
public: 
  ...
  void Configure(const Entity &entity, const std::shared_ptr<const sdf::Element> &_sdf, EntityComponentManager &_ecm, EventManager &) override {
  ...
  void PreUpdate(const UpdateInfo &, EntityComponentManager &ecm) override {
    ...
    for (int i = 0; i < c_vars.num_of_props; i++) {
      auto jointVelCmd = components::JointVelocityCmd(ecm.CreateComponent<components::JointVelocityCmd>(this->entity));
      jointVelCmd.Data()[0] = c_vars.turning_dir(i) * (latest_msg(i) * RPM2RPS / c_vars.slow_down);
      ecm.SetComponentData(this->entity, jointVelCmd);
    }
  }
...
GZ_PLUGIN_ALIAS(BetaMotorPlugin, "BetaMotorPlugin")
