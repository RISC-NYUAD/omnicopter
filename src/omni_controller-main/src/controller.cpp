...
void Controller::send_idle_PWM() {
    // Constants for PWM-force calibration
    float a_vals[8] = {
        128.8304, 105.0259, 110.1237, 110.4929,
        104.4360, 118.3979, 126.6510, 109.5292
    };
    float b_vals[8] = {
        4.5982, 3.8684, 3.5600, 6.1778,
        5.7502, 5.3335, 6.0286, 4.2650
    };
    float c_vals[8] = {
        993.1251, 998.0056, 995.9796, 996.8340,
        996.5013, 996.2702, 994.8973, 995.8504
    };

    float F_idle = 5.0f;  // Constant idle force in Newtons
    //int motor_index = 0;  // Set this to the motor you want to test (0 to 7)
    ...
    std::vector<float> prop_vel;
    for (int i = 0; i < 8; ++i) {
        float pwm = a_vals[1] * std::sqrt(F_idle) + b_vals[1] * F_idle + c_vals[1];
        prop_vel.push_back(pwm);
    }
/*    
    for (int i = 0; i < 8; ++i) {
        if (i != motor_index) {
...
void Controller::full_allocation_actuation() {
	...
	Eigen::Matrix2d H = Eigen::Matrix2d::Identity();
	Eigen::Vector2d f = prop_cmd_d.transpose() * this->nB;
	Eigen::VectorXd x = Utils::solveQP(H, f, this->nB, -1*prop_cmd_d.array()+0.4);
	prop_cmd_d = prop_cmd_d + this->nB * x;
	this->prop_cmd_d = prop_cmd_d.array().max(0);
