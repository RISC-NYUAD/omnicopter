...
def plot_trajectory(trajectory):
    ...
    n_points = 120  # Number of evaluation points
...
v_max = np.array([
    [-0.5, 0.5],  # X-axis linear velocity limits
    [-0.5, 0.5],  # Y-axis linear velocity limits
    [-0.5, 0.5],  # Z-axis linear velocity limits
...
#     max_angular_velocity = np.array([0.3, 0.3, 0.1])  # rad/s
...
def compute_trajectory(path_positions, path_orientations):
    waypoints_pos = np.array(path_positions, dtype=float)
    waypoints_ori = np.array(path_orientations, dtype=float)

    # Keep quaternion sequence sign-continuous
    for i in range(1, waypoints_ori.shape[0]):
        if np.dot(waypoints_ori[i], waypoints_ori[i-1]) < 0:
            waypoints_ori[i:] *= -1

    # --- FEWER KNOTS: no 120-pt upsampling ---
    if len(waypoints_pos) < 2:
        rospy.logwarn("Need at least 2 waypoints; got %d. Returning.", len(waypoints_pos))
        return None

    s_full = np.linspace(0.0, 1.0, len(waypoints_pos))
    N_KNOTS = min(6, len(waypoints_pos))  # try 6; straight line -> 2 works too

    if N_KNOTS < len(waypoints_pos):
        s_knots = np.linspace(0.0, 1.0, N_KNOTS)
        pos_knots = np.vstack([np.interp(s_knots, s_full, waypoints_pos[:, i])
                               for i in range(3)]).T
        key_rots = R.from_quat(waypoints_ori)
        ori_knots = Slerp(s_full, key_rots)(s_knots).as_quat()
    else:
        s_knots = s_full
        pos_knots = waypoints_pos
        ori_knots = waypoints_ori

    path_data = np.hstack((pos_knots, ori_knots))
    path = ta.SplineInterpolator(s_knots, path_data)
    # -----------------------------------------

    # Limits (keep your existing numbers)
    max_linear_velocity     = np.array([0.5, 0.5, 0.5], dtype=float)
    max_linear_acceleration = np.array([0.3, 0.3, 0.3], dtype=float)
    max_angular_velocity     = np.array([0.3, 0.3, 0.1], dtype=float)  # rad/s
    max_angular_acceleration = np.array([0.1, 0.1, 0.1], dtype=float)  # rad/s^2

    def angular_velocity_to_quaternion_rate(ang_vel, q):
        wx, wy, wz = ang_vel
        q0, q1, q2, q3 = q
        q_dot = 0.5 * np.array([
            -q1*wx - q2*wy - q3*wz,
    ...
    def angular_acceleration_to_quaternion_rate_derivative(ang_acc, q):
        ax, ay, az = ang_acc
        q0, q1, q2, q3 = q
        q_ddot = 0.5 * np.array([
            -q1*ax - q2*ay - q3*az,
    ...
    q_rate_limit_lu = np.array([
        [-q_rate_limit[0],  q_rate_limit[0]],
        [-q_rate_limit[1],  q_rate_limit[1]],
        [-q_rate_limit[2],  q_rate_limit[2]],
        [-q_rate_limit[3],  q_rate_limit[3]]
    ], dtype=float)

    v_max = np.array([
        [-max_linear_velocity[0], max_linear_velocity[0]],
        [-max_linear_velocity[1], max_linear_velocity[1]],
        [-max_linear_velocity[2], max_linear_velocity[2]],
        *q_rate_limit_lu
    ], dtype=float)

    a_max = np.array([*max_linear_acceleration, *q_rate_derivative_limit], dtype=float)

    # Use the fixed constraint class (no obstacle callables)
    velocity_constraint     = constraint.JointVelocityConstraint(v_max)
    acceleration_constraint = constraint.JointAccelerationConstraint(a_max)

    instance = algo.TOPPRA([velocity_constraint, acceleration_constraint], path, solver_wrapper='seidel')
    instance.compute_parameterization(0.0, 0.0)
    trajectory = instance.compute_trajectory()
    return trajectory
...
def main():
    ...
    rospy.Subscriber("solution_path", Path, path_callback)
