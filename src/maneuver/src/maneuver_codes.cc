 ...
 */
bool mv_rotateTo(const Maneuver *maneuver, const maneuver::msg::FullPose Pose, double roll, double pitch, double yaw, double duration, Maneuver::pose_sequence *path, const bool moving) {
	bool status;
	...
	if (!moving){
		...
		q.coeffs() <<Pose.pose.orientation.x,Pose.pose.orientation.y,Pose.pose.orientation.z,Pose.pose.orientation.w;
		double roll_, pitch_, yaw_;
		quaternionToRPY(q, roll_, pitch_, yaw_);
	...
	{
		maneuver::msg::FullPose lastPose = path->_buffer.back();
	...
	//RCLCPP_ERROR(rclcpp::get_logger("maneuver"),"P:[%f, %f, %f, %f], [%f, %f, %f, %f]", Pose.pose.position.x, Pose.pose.position.y, Pose.pose.position.z, pose_yaw, x, y, z, yaw);
	...
	if(status == false) {
		RCLCPP_ERROR(rclcpp::get_logger("maneuver"),"path duration error");
		return false;
	}
	if(moving)
	{
	...
	if(status == false) {
		RCLCPP_ERROR(rclcpp::get_logger("maneuver"),"path construction error");
	}
	return status;
}
 ...
 * --- go to a specific position and yaw -----------------------------------------------------
 */
bool mv_goto(const Maneuver *maneuver, const maneuver::msg::FullPose Pose, double x, double y, double z, double yaw, double duration, Maneuver::pose_sequence *path, const bool moving) {
	bool status;
 ...
 */
bool mv_goto_6D(const Maneuver *maneuver, const maneuver::msg::FullPose Pose, double x, double y, double z, 
 ...
 */
bool mv_plan_5DEllipse(const Maneuver *maneuver, const maneuver::msg::FullPose Pose, 
	...
	if(status == false) {
		RCLCPP_ERROR(rclcpp::get_logger("maneuver"),"path duration error at 0.");
		//return false;
	}
	status = mv_sample_path(lpath, path, 1000.0 / maneuver->period);
	if(status == false) {
	...
	// execute ellipse
	status = mv_sample_5DEllipse_path(path, 1000.0 / maneuver->period, u, v, X_mean, roll_start, roll_mid, roll_end, pitch_start, pitch_mid, pitch_end, yaw_, duration);
	if(status == false) {
		RCLCPP_ERROR(rclcpp::get_logger("maneuver"),"path construction error");
	}
 ...
 */
bool mv_plan_flip(const Maneuver *maneuver, const maneuver::msg::FullPose Pose, bool roll_bool, bool pitch_bool, double duration, Maneuver::pose_sequence *path) {
	bool status;
	Eigen::Array3d X;
	...
	X, yaw_, roll_bool, pitch_bool, duration);
 ...
 */
bool mv_plan_take_off(const Maneuver *maneuver, const maneuver::msg::FullPose Pose, double height, double duration, Maneuver::pose_sequence *path) {
	bool status;
...
bool mv_plan_land(const Maneuver *maneuver, const maneuver::msg::FullPose Pose, double height_1, double duration_1, double height_2, double duration_2, Maneuver::pose_sequence *path, const bool moving) {
	bool status;
