...
State::State() : nh_() {
	...
	static tf2_ros::StaticTransformBroadcaster static_br;
  	...
  	ts.header.frame_id = "world";   // parent
 	 ts.child_frame_id  = "map";     // child
...
#include <tf2_geometry_msgs/tf2_geometry_msgs.h> // toMsg/fromMsg

void State::publishFullPose() {
  // --- Publish pose_full every loop (500 Hz as you wanted) ---
  this->full_pose.pose  = this->pose_;     // <-- base pose: world -> base (Omnicopter)
  this->full_pose.vel   = this->twist_;
  this->full_pose.acc   = this->accel_;
  this->full_pose.header.stamp    = this->state_last_time;
  this->full_pose.header.frame_id = "world";
  this->full_pose.header.seq      = seqID++;
  this->pose_pub.publish(this->full_pose);

  // ========= Base -> Camera extrinsics (camera is 0.16 m along -Y, facing -Y) =========
  // Translation in base frame:
  const double cx = 0.0;
  const double cy = -0.16;   // 16 cm along -Y
  const double cz = 0.0;

  // Rotation in base frame: yaw = -90 deg to make camera look along -Y
  tf2::Quaternion q_bc;
  q_bc.setRPY(0.0, 0.0, -1.571); // roll=0, pitch=0, yaw=-90deg

  tf2::Transform T_b_c(q_bc, tf2::Vector3(cx, cy, cz)); // base->camera

  // ========= Compose world -> camera: T_w_c = T_w_b * T_b_c =========
  tf2::Transform T_w_b; // world->base from your pose_
  {
    tf2::Quaternion q_wb;
    tf2::fromMsg(this->pose_.orientation, q_wb);
    T_w_b.setOrigin(tf2::Vector3(this->pose_.position.x,
                                 this->pose_.position.y,
                                 this->pose_.position.z));
    T_w_b.setRotation(q_wb);
  }

  const tf2::Transform T_w_c = T_w_b * T_b_c;
  //const tf2::Transform T_w_c = T_w_b;
  // Convert to geometry_msgs::Pose for /odom
  geometry_msgs::Pose cam_pose;
  cam_pose.position.x = T_w_c.getOrigin().x();
  cam_pose.position.y = T_w_c.getOrigin().y();
  cam_pose.position.z = T_w_c.getOrigin().z();
  cam_pose.orientation = tf2::toMsg(T_w_c.getRotation());

  // --- Publish /odom every loop (world -> camera_link) ---
  nav_msgs::Odometry odom;
  odom.header.stamp    = this->state_last_time;
  odom.header.frame_id = "world";         // parent
  odom.child_frame_id  = "camera_link";   // child

  odom.pose.pose  = cam_pose;

  // NOTE: If your twist_ is expressed in the base frame but you publish child= camera_link,
  // you may want to rotate it into camera frame. For now we pass it through as-is:
  odom.twist.twist = this->twist_;

  // Covariances (same as before)
  const double pvar = 1e-6, ovar = 1e-6, tvar = 1e-4;
  odom.pose.covariance[0]  = pvar;  odom.pose.covariance[7]  = pvar;  odom.pose.covariance[14] = pvar;
  odom.pose.covariance[21] = ovar;  odom.pose.covariance[28] = ovar;  odom.pose.covariance[35] = ovar;
  odom.twist.covariance[0] = tvar;  odom.twist.covariance[7] = tvar;  odom.twist.covariance[14] = tvar;
  odom.twist.covariance[21]= tvar;  odom.twist.covariance[28]= tvar;  odom.twist.covariance[35]= tvar;

  this->odom_pub.publish(odom);

  // --- Broadcast TF world -> camera_link ONLY when stamp advances (avoid TF_REPEATED_DATA) ---
  static ros::Time last_tf_stamp(0);
  if (!this->state_last_time.isZero() && this->state_last_time != last_tf_stamp) {
    geometry_msgs::TransformStamped ts;
    ts.header.stamp    = this->state_last_time;
    ts.header.frame_id = "world";
    ts.child_frame_id  = "camera_link";

    ts.transform.translation.x = cam_pose.position.x;
    ts.transform.translation.y = cam_pose.position.y;
    ts.transform.translation.z = cam_pose.position.z;
    ts.transform.rotation      = cam_pose.orientation;

    tf_br_.sendTransform(ts);
    last_tf_stamp = this->state_last_time;
  }
}
...
void State::IMU_callback()
{
    ...
    tf::Quaternion q(q1, q2, q3, q0);
				...
				//std::cout << roll << " " << pitch << " " << yaw << "; " << roll_ << " " << pitch_ << " " << yaw_ << std::endl;
				//} else{
