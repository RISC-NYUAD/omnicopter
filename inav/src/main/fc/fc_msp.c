    ...
    case MSP_RAW_IMU:
        {
            ...
            //sbufWriteU16(dst, (int16_t)(gpsSol.velNED[X])); // return gps groundSpeed
           // sbufWriteU16(dst, (int16_t)(gpsSol.velNED[Y])); // return gps groundCourse
...
//#endif
    ...
    // Ensure the RPM value is valid before writing
    if (escState && escState->rpm > 0) {
        sbufWriteU32(dst, escState->rpm);
    } else {
        sbufWriteU32(dst, 0); // Send 0 if no valid telemetry
    }
}
